...
namespace dpl
{
	...
	protected: // lifecycle
		...
		inline Variation&				operator=(					Swap<VariationT>						other)
		{
	...
	public: // functions
		template<typename T>
		inline static uint32_t			get_typeID()
		{
			return VariantHandler::template get_typeID<T>();
		}

		inline uint32_t					get_numVariants() const
		{
			return static_cast<uint32_t>(m_variants.size());
		}

		inline VariantT*				find_base_variant(			const uint32_t							TYPE_ID)
		{
		...
		inline const VariantT*			find_base_variant(			const uint32_t							TYPE_ID) const
		{
			auto it = m_variants.find(VariantHandler::get_typeInfo(TYPE_ID));
			return (it != m_variants.end()) ? static_cast<const VariantT*>(it->second.get()) : nullptr;
		}

		inline VariantT*				find_base_variant(			const std::string&						VARIANT_NAME)
		{
			return Variation::find_base_variant(VariantHandler::get_typeID(VARIANT_NAME));
		}

		inline const VariantT*			find_base_variant(			const std::string&						VARIANT_NAME) const
		{
			return Variation::find_base_variant(VariantHandler::get_typeID(VARIANT_NAME));
		}

		inline bool						has_variant(				const uint32_t							TYPE_ID) const
		{	
		...
		template<typename T>
		inline const T*					find_variant() const
		{
		...
		template<typename T>
		inline const T&					get_variant() const
		{
	...
	protected: // functions
		...
		template<typename T>
		inline T&						get_variant()
		{
			return *find_variant<T>();
		}

		inline Result<VariantT*>		create_default_variant(		const uint32_t							TYPE_ID)
		{
		...
		inline Result<VariantT*>		create_default_variant(		const std::string&						VARIANT_NAME)
		{
			return Variation::create_default_variant(VariantHandler::get_typeID(VARIANT_NAME));
		}

		template<typename T, typename... CTOR>
		inline Result<T*>				create_variant(				CTOR&&...								args)
		{
		...
		template<typename T>
		inline bool						destroy_variant()
		{
			return destroy_variant_internal(typeid(T));
		}

		inline bool						destroy_variant(			const uint32_t							TYPE_ID)
		{
		...
		*/
		template<typename T>
		inline bool						destroy_all_variants_except()
		{
			bool bResult = false;
		...
		inline bool						destroy_all_variants()
		{
		...
		inline void						iterate(					std::function<void(VariantT&)>			function)
		{
		...
		inline void						iterate(					std::function<void(const VariantT&)>	function) const
		{
			for(auto& it : m_variants)
			{
				function(*it.second->cast());
		...
		inline void						for_each_variant(			std::function<void(VariantT&)>			function)
		{
			iterate(function);
		}

		inline void						for_each_variant(			std::function<void(const VariantT&)>	function) const
		{
			iterate(function);
		}

	private: // functions
		inline bool						destroy_variant_internal(	const std::type_index&					TYPE_INDEX)
		{
		...
		inline MyIterator				destroy_it_internal(		MyIterator&								iterator)
		{
			return m_variants.erase(iterator);
		}

	private: // functions
		inline VariationT*				cast()
		{
			return static_cast<VariationT*>(this);
		}

		inline const VariationT*		cast() const
		{
			return static_cast<const VariationT*>(this);
		}

		inline void						bind_variants()
		{
			for(auto& it : m_variants)
			{
